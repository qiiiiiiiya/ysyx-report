...
int reg_name2idx(const char *reg_name);
...
static long eval(int p, int q,bool *success) {
    ...
    if (p == q) {  // 单个token处理（数字、寄存器）
        switch (tokens[p].type) {
            ...
            case TK_REG: {  // 寄存器（如$eax）
                char *reg_name = tokens[p].str + 1;  // 去掉'$'
                int idx = reg_name2idx(reg_name);
                if (idx == -1) {
                    fprintf(stderr, "错误：无效寄存器 %s\n", tokens[p].str);
                    *success = false;
                    return 0;
                }
                return cpu.gpr[idx];  // 假设cpu.regs存储寄存器值
            }
            default:
                *success = false;
                return 0;
