...
static struct rule {
    ...
    {"\\$?[a-z][a-z0-9]{0,3}", TK_REG},  // 寄存器匹配规则（带或不带$）
    {" +", TK_NOTYPE},                    // 空格
    {"\\(", TK_LPAREN},                   // 左括号
    {"\\)", TK_RPAREN},                   // 右括号
    {"\\+", TK_PLUS},                     // 加号
...
static word_t eval(int p, int q, bool *success) {
    ...
    if (tokens[p].type == TK_MINUS_F || tokens[p].type == TK_DEREF) {
        ...
        switch (tokens[p].type) {
            ...
            case TK_DEREF:   return vaddr_read(val, 4); // 32位内存读取
            default:         return 0;
        }
    }
    // 单token处理
